...
do
	...
	then
		...
		URL=$(curl -X POST -H 'Content-Type: application/x-www-form-urlencoded' -d "SEARCH=${art}&dummy=" http://www.reichelt.de/index.html\?\&ACTION\=446\&LA\=3 2>/dev/null | grep '<a class="al_artinfo_link" itemprop="url"' | awk '{ print $4 }' | sed "s/href=//g" | sed 's/"//g')
		xdg-open $URL
