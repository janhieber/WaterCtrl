 ...
 *--------------------------------------------------------------------------------+
*/
static int vfprintf_(void (*putc_)(char), const char* str,  va_list arp)
{
	...
	while ((d = *str++) != 0) {
		...
		if (l==0) {
			if (r > 0){      //unsigned
				long_itoa((unsigned long)va_arg(arp, int), r, w, (putc_)); //needed for 16bit int, no harm to 32bit int
			}
			else            //signed
				long_itoa((long)va_arg(arp, int), r, w, (putc_));
		} else if (l==1){  // long =32bit
				long_itoa((long)va_arg(arp, long), r, w, (putc_));        //no matter if signed or unsigned
		}
	}
...
void long_itoa (long val, int radix, int len, void (*putc_) (char))
{
	char c, sgn = 0, pad = ' ';
	char s[20];
	int  i = 0;
